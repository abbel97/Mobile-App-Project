	...
	Widget build(BuildContext context) {
		return Scaffold(
			...
			body: SafeArea(
                ...
                children: [Row(children: [Container(width: 34, height: 34,
                 decoration: BoxDecoration(color: AppColors.neutral, 
                 borderRadius: BorderRadius.circular(10)), 
                 child: const Icon(Icons.person, color: AppColors.primary, size: 20)),
                  const SizedBox(width: 14), Text('HOME-TWEAK', style: AppTextStyles.titleMedium.copyWith(fontSize: 22, color: AppColors.primary))]), 
                  const SizedBox(height: 30), Text('Check recent posted\njobs', style: AppTextStyles.headline2.copyWith(fontSize: 38, height: 1.05, color: AppColors.textPrimary)), 
...
class _Pill extends StatelessWidget {
	...
	@override
	Widget build(BuildContext context) {
		return Container(padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 12), 
    decoration: BoxDecoration(color: selected ? AppColors.primary : AppColors.card, borderRadius: BorderRadius.circular(6)), 
    child: Text(label, style: AppTextStyles.titleSmall.copyWith(fontSize: 16, color: selected ? AppColors.surface : AppColors.textBody))
    );
	}
}
